...
function Avatar({ name, online, size = 'md' }: { name: string; online?: boolean; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${sz} ${bg} rounded-full flex items-center justify-center text-white font-bold relative`}>
      {initials}
      {online !== undefined && (
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-xs font-semibold text-slate-700">OrthoAI</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-bold">IA</span>
      ...
      <div className={`flex-1 max-w-[75%] flex flex-col gap-1 ${isMe ? 'items-end' : 'items-start'}`}>
        {!isMe && (
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${ROLE_COLORS[msg.userRole]}`}>
              {ROLE_LABELS[msg.userRole]}
            </span>
            <span className="text-[10px] text-slate-400">{formatTime(msg.createdAt)}</span>
          </div>
        )}
        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? 'bg-[#0056b3] text-white rounded-tr-sm' : 'bg-slate-100 text-slate-800 rounded-tl-sm'}`}
          dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content) }} />
        {isMe && <span className="text-[10px] text-slate-400">{formatTime(msg.createdAt)}</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-100 bg-white">
        <span className="text-xs text-emerald-600 font-semibold">{onlineCount} online</span>
        ...
        <span className="text-xs font-semibold text-slate-700 ml-auto">Chat clínico do caso</span>
      </div>
  ...
  return (
    <div className="h-full flex flex-col p-4 space-y-4 overflow-y-auto">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-slate-800">Equipe do Caso</h3>
          <p className="text-xs text-slate-500">
            {collabs.filter(c => c.status === 'accepted').length + 1} participantes · {collabs.filter(c => c.status === 'pending').length} convites pendentes
          </p>
        </div>
...
function SharedViewerTab({ caseId }: { caseId: string }) {
  ...
  if (!activeCase?.imageUrl) return (
    <div className="h-full flex flex-col items-center justify-center text-center p-8">
      ...
      <h3 className="text-sm font-bold text-slate-700 mb-1">Nenhuma imagem disponível</h3>
      <p className="text-xs text-slate-500">Adicione imagens ao caso para usar a visualização compartilhada.</p>
    </div>
  );
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-100 bg-white">
        <h3 className="text-sm font-bold text-slate-800">Visualização Compartilhada</h3>
        {collabs.length > 0 && (
          <span className="text-xs text-emerald-600 font-semibold">{collabs.length + 1} assistindo</span>
        )}
      </div>
    ...
    <div className="flex items-center gap-4 px-4 py-3 bg-white border-b border-slate-100">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500 font-mono truncate">
          {c.patientName} · {SPECIES_LABELS[c.species]} · {c.breed} · {c.ageYears}a {c.weightKg}kg · {PROCEDURE_LABELS[c.procedure]}
        </p>
      </div>
      {c.precisionScore && (
        <div className="text-center">
          <PrecisionGauge value={c.precisionScore} size={52} />
          <p className="text-[10px] text-slate-500 mt-1">Precisão IA</p>
        </div>
      )}
    </div>
  );
...
type Tab = 'chat' | 'specialists' | 'viewer' | 'orthoai';
...
const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
  ...
  { id: 'orthoai', label: 'OrthoAI Consultor', icon: Bot },
];
...
export default function CasePage() {
  ...
  if (!activeCase) return (
    <div className="h-full flex flex-col items-center justify-center">
      <p className="text-sm text-slate-500 mb-4">Nenhum caso selecionado.</p>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-white border-b border-slate-100">
        ...
        <span className="text-[10px] text-emerald-600 font-semibold">Colaboração ativa</span>
        {activeCase.status === 'critical' && (
          <>
            <AlertTriangle size={10} className="text-red-500" />
            <span className="text-[10px] text-red-600 font-bold">CRÍTICO</span>
          </>
        )}
        <button
          onClick={() => {
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {tab === 'orthoai' && (
          <div className="p-4 overflow-y-auto h-full">
            <CaseAIPanel caseId={activeCase.id} />
          </div>
        )}
      </div>
    </div>
