...
export async function generateMonthlyReport(
  ...
  doc.text(`Nome: ${user.name}`, 14, y); y += 5;
  doc.text(`CRMV: ${user.crmv}`, 14, y); y += 5;
  doc.text(`Especialidade: ${user.specialty}`, 14, y); y += 5;
  doc.text(`Instituição: ${user.institution}`, 14, y); y += 10;
  ...
  metrics.forEach(m => {
    ...
    doc.text(`• ${m.label}: ${val} (${trend})`, 14, y); y += 6;
  });
  y += 4;
  ...
  let x = 14;
  doc.setFillColor(0, 86, 179); doc.setTextColor(255,255,255); doc.setFontSize(9); doc.setFont('helvetica','bold');
...
export async function generateCaseReport(c: ClinicalCase, user: User): Promise<void> {
  ...
  let y = 54;
  // Patient
  doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(0,86,179); doc.text('Dados do Paciente', 14, y); y += 7;
  doc.setFontSize(9); doc.setFont('helvetica','normal'); doc.setTextColor(0,0,0);
  const patientFields = [
  ...
  // AI Analysis
  if (c.aiAnalysis) {
    doc.setFontSize(11); doc.setFont('helvetica','bold'); doc.setTextColor(0,86,179); doc.text('Análise IA — OrthoVision', 14, y); y += 7;
    doc.setFontSize(9); doc.setFont('helvetica','normal'); doc.setTextColor(0,0,0);
    ...
    c.aiAnalysis.anatomicalLandmarks.forEach(l => {
      ...
      doc.text(`• ${l.name}: ${status}`, 14, y); y += 5;
    });
    y += 5;
