...
export function Badge({ children, variant = 'default', className = '' }: { children: React.ReactNode; variant?: 'default' | 'success' | 'warning' | 'danger' | 'info' | 'blue'; className?: string }) {
  const styles: Record<string, string> = {
    ...
    danger: 'bg-red-50 text-red-700 border border-red-200 animate-pulse',
...
export function Button({ children, variant = 'primary', size = 'md', loading, className = '', disabled, ...props }: BtnProps) {
  ...
  return <button className={`${base} ${variants[variant]} ${sizes[size]} ${className}`} disabled={disabled || loading} {...props}>{loading && <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}{children}</button>;
}
...
export const KPIWidget = React.memo(function KPIWidget({ label, value, unit, trend, trendDirection, color }: { label: string; value: number | string; unit?: string; trend: number; trendDirection: 'up' | 'down' | 'stable'; color: string }) {
  ...
  return (
    <Card className="p-4 relative overflow-hidden">
      ...
      <p className="text-2xl font-bold font-mono mt-1">{value}{unit && <span className="text-sm text-slate-500 ml-1">{unit}</span>}</p>
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      <div>
        ...
        {subtitle && <p className="text-slate-700 font-medium mt-0.5 text-sm">{subtitle}</p>}
      </div>
      {action}
    </div>
  );
...
export function PrecisionGauge({ value, size = 80 }: { value: number; size?: number }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 flex items-center justify-center text-xs font-bold font-mono" style={{ color }}>{value.toFixed(0)}%</span>
    </div>
  );
}
...
export function RiskTag({ level }: { level: 'low' | 'medium' | 'high' }) {
  const map = {
    ...
    high: { label: 'Alto', cls: 'bg-red-50 text-red-700' },
  };
